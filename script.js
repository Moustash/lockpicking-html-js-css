...
$(function(){
  ...
  $('body').on('mousemove', function(e){
    if (lastMousePos && !gameOver && !gamePaused) {
      ...
      pinRot += pinRotChange;
      pinRot = Util.clamp(pinRot,maxPinRot,minPinRot);
      pin.css({
  ...
  $('body').on('keydown', function(e){  
    if ( (e.keyCode == 69) && !userPushingCyl && !gameOver && !gamePaused) {
      pushCyl();
    }
  });
  
  $('body').on('keyup', function(e){
    if ( (e.keyCode == 69) && !gameOver) {
      unpushCyl();
    }
  });
...
function pushCyl() {
  var distFromSolve, cylRotationAllowance;
      ...
      // set an interval based on keyrepeat that will rotate the cyl forward, 
      // and if cyl is at or past maxCylRotation based on pick distance from solve, 
      // display "bounce" anim and do damage to pick. 
      // If pick is within sweet spot params, allow pick to rotate to maxRot and trigger solve functionality
      
      //SO...to calculate max rotation, we need to create a linear scale from solveDeg+padding to maxDistFromSolve - 
      // if the user is more than X degrees away from solve zone, they are maximally distant and the cylinder cannot travel at all. 
      // Let's start with 45deg. So...we need to create a scale and do a linear conversion. If user is at or beyond max, return 0. 
      // If user is within padding zone, return 100. Cyl may travel that percentage of maxRot before hitting the damage zone.
      
      distFromSolve = Math.abs(pinRot - solveDeg) - solvePadding;
      distFromSolve = Util.clamp(distFromSolve, maxDistFromSolve, 0);

     //  oldval is distfromsolve, oldmin is....0? oldMax is maxDistFromSolve, newMin is 100 (we are at solve, so cyl may travel 100% of maxRot), 
     //  newMax is 0 (we are at or beyond max dist from solve, so cyl may not travel at all - UPDATE - must give cyl just a teensy bit of travel so user isn't hammered);

      cylRotationAllowance = Util.convertRanges(distFromSolve, 0, maxDistFromSolve, 1, 0.02);
      cylRotationAllowance = cylRotationAllowance * maxCylinderRot;
      
      cylRotationInterval = setInterval(function(){
        cylRot += cylRotSpeed;
        if (cylRot >= maxCylinderRot) {
          cylRot = maxCylinderRot;
